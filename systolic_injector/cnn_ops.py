...
def kernel2matrix(kernels):
    ...
    # First, stack all unfolded kernels row by row
    output = np.zeros((kernel_num,output_height))
    for idx in range(kernel_num):
        output[idx,:] = kernels[:,:,idx].flatten()
    return np.transpose(output)

# NOTE: instead of np.dot() function there should be a function that sends the
# data via AXI bus to the accelerator and reads back the result
def matmul(a, b, mac_num):
    ...
    return result
"""
Salvo, this part is the one we were discussing the other day
"""
def zero_padding(x, dim):
    ...
    output = np.zeros((input_size,output_size))
...
def conv_2d(x, kernel_num, kernels, bias_vector, mac_num, stride = 1):
    ...
    output = np.zeros((output_width,output_width,output_depth))
