    ...
    class TestMatmulsFloat32(unittest.TestCase):
        ...
        def test_float32_noinjection(self):
            ...
            for intype in [0, 1, 2]:
                ...
                for a_scale in range(-100, 100):
                    for b_scale in range(-100, 100):
                        ...
                        A = np.array(
                                np.random.rand(N1,N3) * 2 ** a_scale,
                                dtype=np.float32
                            )
                        B = np.array(
                                np.random.rand(N3,N2) * 2 ** b_scale,
                                dtype=np.float32
                            )
